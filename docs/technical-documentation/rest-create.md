...
1. `-H"Content-Location: public://images/my-image.png"` - store the file in the public scheme (ie. in Drupal) at the path `images/my-image.png`. To store the file in Fedora use the `fedora//` scheme (ie. fedora://images/my-image.png). Only applies to newly created media. If the media exists, the file content in `--data-binary` will replace the content at the existing file's location.
