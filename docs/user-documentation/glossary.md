...
## CLAW
 CLAW (CLAW Linked Asset WebDataFrameWork) was the production code name for the software released in June, 2019 as _Islandora 8_
...
## Drupal Roles
Roles are a way of assigning specific permissions to a group of users. Any user assigned to a role will have the same permissions as all other users assigned to that role. This allows you to control which users have permission to view, edit, or delete content in both Drupal and the Fedora repository.
...
## Flysystem
Flysystem is a filesystem abstraction library for PHP. Islandora uses Flystsem to swap about different backend filesystem applications, and comes with a custom Flystsem adapter for Fedora.
...
## View 
A Drupal term for a formatted listing of data; typically, the data comes from content entities.
